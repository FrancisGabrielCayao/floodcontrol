...
const ESP32_IP = "192.168.1.10"; // CHANGE THIS to your ESP32 IP from Serial Monitor
...
function updateBatteryChart(adcValue) {
    // Math: 12-bit ADC (4095) + 3.3V Ref + 2x Voltage Divider (100k resistors)
    const pinVoltage = (adcValue / 4095.0) * 3.3;
    const batteryVoltage = pinVoltage * 2.0; 
    
    // Percentage for Li-ion (3.2V empty to 4.2V full)
    let percentage = Math.round(((batteryVoltage - 3.2) / (4.2 - 3.2)) * 100);
